    ...
    <div class="mx-auto max-w-4xl px-4 py-10">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <h1 class="mt-2 text-3xl font-black text-slate-900">Edit Expense</h1>
            </div>
            <a href="{{ request('return_to', route('expenses.index')) }}"
                class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Back to Expenses</a>
        </div>
        ...
        <form method="POST" action="{{ route('expenses.update', $expense) }}" id="expense-form"
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="type" id="expense-type"
                        class="w-full rounded-lg border border-slate-300 px-3 py-2">
                        <option value="meal" {{ old('type', $expense->type) === 'meal' ? 'selected' : '' }}>Meal Cost
                            (affects meal rate)</option>
                        <option value="fixed" {{ old('type', $expense->type) === 'fixed' ? 'selected' : '' }}>Shared
                            Cost (deducts balance only)</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $expense->category) }}" required
                    placeholder="Rice, Fish, Oil, Milk, Rent, Utility..."
                    class="w-full rounded-lg border border-slate-300 px-3 py-2">
            </div>
                    ...
                    @foreach ($members as $member)
                        <div class="flex items-center gap-3 rounded-lg border border-slate-200 bg-white p-3">
                            <input type="checkbox" name="member_ids[]" value="{{ $member->id }}"
                                {{ in_array($member->id, old('member_ids', $expense->member_ids ?? []), true) ? 'checked' : '' }}
                            ...
                            <input type="number" name="member_shares[{{ $member->id }}]" min="0"
                                step="0.01"
                                value="{{ old('member_shares.' . $member->id, $expense->member_shares[$member->id] ?? 0) }}"
                                data-member-share
            ...
            <div>
                ...
                <textarea name="note" rows="3" class="w-full rounded-lg border border-slate-300 px-3 py-2">{{ old('note', $expense->note) }}</textarea>
            </div>
    ...
    <script>
        ...
        function distributeAmount() {
            ...
            memberShares.forEach(input => {
                ...
                if (checkbox.checked && (!input.value || parseFloat(input.value) === 0 || input.dataset.autofill ===
                        'true')) {
                    input.value = share;
        ...
        document.getElementById('expense-form').addEventListener('submit', event => {
            if (expenseType.value === 'fixed') {
                ...
                if (Math.abs(total - amount) > 0.01) {
                    ...
                    updateAllocation();
                    window.toast.error('সব selected member amount-এর মোট অবশ্যই Total Amount-এর সমান হতে হবে।',
                        'অ্যালোকেশন মেলেনি');
                }
            }
        });
